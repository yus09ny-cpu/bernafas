...
// regular app's AuthScreen.tsx, even though both reuse the same Supabase
// Auth session underneath. Two methods, per explicit spec (2026-08-21
// correction — the first build of this screen used magic-link instead of
// real email+password, which was WRONG: the spec named "emel/kata laluan"
// explicitly and magic-link is a different, unasked-for mechanism, not a
// substitute; see the correction for why "reuse a similar-looking existing
// pattern" isn't the same as "matches what was asked for"):
// - Google OAuth (unchanged, reused as-is from the main app).
// - Real email+password (supabase.auth.signInWithPassword) — NOT
//   magic-link. Registration (AffiliateRegisterScreen.tsx) is where the
//   password is actually set; this screen only signs in with it.
//
// After a successful sign-in (either method), this screen's job is
// resolving that session to an AFFILIATE record (api/affiliate.ts's
// action=me, auto-links on first login) and landing on the existing
// /affiliate/dashboard/:id screen, not App.tsx's tab shell — an affiliate
// isn't necessarily a Bernafas app user (0003_affiliate_program.sql's
// header).
//
...
export default function AffiliateLoginScreen() {
  const { status, error: authError, signInWithGoogle, signInWithPassword, sendPasswordReset } = useAuth()
  ...
  const handleForgotPassword = async () => {
    if (!email.trim() || resetSubmitting) return
    ...
    >
      ...
      <div className="flex w-full max-w-xs flex-col items-center gap-8">
        ...
        <div className="flex flex-col items-center gap-2">
          <h1 className="text-xl font-bold text-[var(--color-text)]">Log masuk ke dashboard affiliate</h1>
          <p className="max-w-xs text-sm leading-relaxed text-[var(--color-text-muted)]">
            Guna e-mel dan kata laluan yang sama semasa anda mendaftar.
          </p>
          {authError && <p className="max-w-xs text-sm text-[var(--color-warm)]">{authError}</p>}
          {resetSent && (
            <p className="max-w-xs text-sm text-[var(--color-primary-dark)]">
              Pautan set semula kata laluan telah dihantar ke {email}.
            </p>
          )}
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex w-full flex-col gap-3">
          ...
          >
            {submitting ? 'Log masuk...' : 'Log Masuk'} <ArrowRight size={18} />
          </button>
          <button
            type="button"
            onClick={handleForgotPassword}
            disabled={resetSubmitting || !email.trim()}
            className="text-xs text-[var(--color-text-muted)] underline-offset-4 hover:underline disabled:opacity-40"
          >
            {resetSubmitting ? 'Menghantar...' : 'Lupa kata laluan?'}
          </button>
        </form>
